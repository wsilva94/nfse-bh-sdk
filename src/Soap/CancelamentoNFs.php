        ...
        if (is_object($this->wsResponse) && isset($this->wsResponse->outputXML)) {
            ...
            if (count($retornoCancelamento) > 0) {
                foreach ($retornoCancelamento as $cancelamento) {
                    ...
                    $this->dataCancelamento['cancelamento'][$cancelamento->Confirmacao->Pedido->InfPedidoCancelamento->IdentificacaoNfse->Numero->__toString()]['codigoCancelamento'] = (int) $cancelamento->Confirmacao->Pedido->InfPedidoCancelamento->CodigoCancelamento;
